...
function CurrentProjectID() {
	...
	return "{0F066488-51F4-4512-9E75-792816ED19E6}";
}
	...
	public function validateUser(&$usr, &$pwd, $autologin, $provider = "")
	{
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ComparePassword($rs->fields('Passcode'), $pwd);
					if ($valid) {
						$this->_isLoggedIn = TRUE;
						$_SESSION[SESSION_STATUS] = "login";
						$_SESSION[SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('JobWorkerID')); // Load user name

						// Call User Validated event
						$row = $rs->fields;
						$UserProfile->assign($row);
						$UserProfile->delete('Passcode'); // Delete password
