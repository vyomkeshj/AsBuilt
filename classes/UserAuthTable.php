...
/**
 * Table class for UserAuthTable
 */
class UserAuthTable extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'UserAuthTable';
		$this->TableName = 'UserAuthTable';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`UserAuthTable`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// UserId
		$this->_UserId = new DbField('UserAuthTable', 'UserAuthTable', 'x__UserId', 'UserId', '`UserId`', '`UserId`', 3, -1, FALSE, '`UserId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// UserName
		$this->UserName = new DbField('UserAuthTable', 'UserAuthTable', 'x_UserName', 'UserName', '`UserName`', '`UserName`', 200, -1, FALSE, '`UserName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->UserName->Sortable = TRUE; // Allow sort
		$this->fields['UserName'] = &$this->UserName;

		// Passcode
		$this->Passcode = new DbField('UserAuthTable', 'UserAuthTable', 'x_Passcode', 'Passcode', '`Passcode`', '`Passcode`', 200, -1, FALSE, '`Passcode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Passcode->Sortable = TRUE; // Allow sort
		$this->fields['Passcode'] = &$this->Passcode;

		// AccessLevel
		$this->AccessLevel = new DbField('UserAuthTable', 'UserAuthTable', 'x_AccessLevel', 'AccessLevel', '`AccessLevel`', '`AccessLevel`', 3, -1, FALSE, '`AccessLevel`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`UserAuthTable`";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('UserId', $row) ? $row['UserId'] : NULL) : $this->_UserId->CurrentValue;
		if (!is_numeric($val))
	...
	{
		global $Language;
		if ($pageName == "UserAuthTableview.php")
			return $Language->phrase("View");
		elseif ($pageName == "UserAuthTableedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "UserAuthTableadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("UserAuthTableview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("UserAuthTableview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "UserAuthTableadd.php?" . $this->getUrlParm($parm);
		else
			$url = "UserAuthTableadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("UserAuthTableedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("UserAuthTableadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("UserAuthTabledelete.php", $this->getUrlParm());
	}
