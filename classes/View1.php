...
class View1 extends DbTable
{
	...
	public function __construct()
	{
		...
		// LeadID
		$this->LeadID = new DbField('View1', 'View1', 'x_LeadID', 'LeadID', '`LeadID`', '`LeadID`', 3, -1, FALSE, '`LeadID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// SiteLocation
		$this->SiteLocation = new DbField('View1', 'View1', 'x_SiteLocation', 'SiteLocation', '`SiteLocation`', '`SiteLocation`', 201, -1, FALSE, '`SiteLocation`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->SiteLocation->Nullable = FALSE; // NOT NULL field
		$this->SiteLocation->Required = TRUE; // Required field
		$this->SiteLocation->Sortable = TRUE; // Allow sort
		$this->fields['SiteLocation'] = &$this->SiteLocation;

		// Suburb
		$this->Suburb = new DbField('View1', 'View1', 'x_Suburb', 'Suburb', '`Suburb`', '`Suburb`', 200, -1, FALSE, '`Suburb`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Suburb->Nullable = FALSE; // NOT NULL field
		$this->Suburb->Required = TRUE; // Required field
		$this->Suburb->Sortable = TRUE; // Allow sort
		$this->fields['Suburb'] = &$this->Suburb;

		// ExpectedStart
		$this->ExpectedStart = new DbField('View1', 'View1', 'x_ExpectedStart', 'ExpectedStart', '`ExpectedStart`', CastDateFieldForLike('`ExpectedStart`', 0, "DB"), 135, 0, FALSE, '`ExpectedStart`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateTaken
		$this->DateTaken = new DbField('View1', 'View1', 'x_DateTaken', 'DateTaken', '`DateTaken`', CastDateFieldForLike('`DateTaken`', 0, "DB"), 135, 0, FALSE, '`DateTaken`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->DateTaken->Nullable = FALSE; // NOT NULL field
		$this->DateTaken->Required = TRUE; // Required field
		$this->DateTaken->Sortable = TRUE; // Allow sort
		...
		// TakenBy
		$this->TakenBy = new DbField('View1', 'View1', 'x_TakenBy', 'TakenBy', '`TakenBy`', '`TakenBy`', 200, -1, FALSE, '`TakenBy`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->TakenBy->Nullable = FALSE; // NOT NULL field
		$this->TakenBy->Required = TRUE; // Required field
		$this->TakenBy->Sortable = TRUE; // Allow sort
		$this->fields['TakenBy'] = &$this->TakenBy;

		// IsComplete
		$this->IsComplete = new DbField('View1', 'View1', 'x_IsComplete', 'IsComplete', '`IsComplete`', '`IsComplete`', 16, -1, FALSE, '`IsComplete`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// CustomerName
		$this->CustomerName = new DbField('View1', 'View1', 'x_CustomerName', 'CustomerName', '`CustomerName`', '`CustomerName`', 200, -1, FALSE, '`CustomerName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CustomerName->Sortable = TRUE; // Allow sort
		$this->fields['CustomerName'] = &$this->CustomerName;

		// CustomerID
		$this->CustomerID = new DbField('View1', 'View1', 'x_CustomerID', 'CustomerID', '`CustomerID`', '`CustomerID`', 3, -1, FALSE, '`CustomerID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Address
		$this->Address = new DbField('View1', 'View1', 'x_Address', 'Address', '`Address`', '`Address`', 200, -1, FALSE, '`Address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Coments
		$this->Coments = new DbField('View1', 'View1', 'x_Coments', 'Coments', '`Coments`', '`Coments`', 201, -1, FALSE, '`Coments`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Coments->Sortable = TRUE; // Allow sort
		$this->fields['Coments'] = &$this->Coments;
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->Coments->DbValue = $row['Coments'];
	}
	...
	{
		return "`LeadID` = @LeadID@ AND `CustomerID` = @CustomerID@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('CustomerID', $row) ? $row['CustomerID'] : NULL) : $this->CustomerID->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
	...
	public function getViewUrl($parm = "")
	{
		if ($parm <> "")
			...
			$url = $this->keyUrl("View1view.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",CustomerID:" . JsonEncode($this->CustomerID->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->CustomerID->CurrentValue != NULL) {
			$url .= "&CustomerID=" . urlencode($this->CustomerID->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getRecordKeys()
	{
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // LeadID
					continue;
				if (!is_numeric($key[1])) // CustomerID
					continue;
				$ar[] = $key;
			}
		}
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->LeadID->CurrentValue = $key[0];
			$this->CustomerID->CurrentValue = $key[1];
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->Coments->setDbValue($rs->fields('Coments'));
	}
	...
	public function renderListRow()
	{
		...
		// Coments
		$this->Coments->ViewValue = $this->Coments->CurrentValue;
		$this->Coments->ViewCustomAttributes = "";
		...
		// Coments
		$this->Coments->LinkCustomAttributes = "";
		$this->Coments->HrefValue = "";
		$this->Coments->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// Coments
		$this->Coments->EditAttrs["class"] = "form-control";
		$this->Coments->EditCustomAttributes = "";
		$this->Coments->EditValue = $this->Coments->CurrentValue;
		$this->Coments->PlaceHolder = RemoveHtml($this->Coments->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->Coments);
				} else {
					$doc->exportCaption($this->LeadID);
					$doc->exportCaption($this->Suburb);
					$doc->exportCaption($this->ExpectedStart);
